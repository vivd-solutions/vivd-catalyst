...
2. **Demote `memory-store`.** Move `InMemoryPlatformStore` to a subpath export of the contracts package (`@agent-chat-platform/core/testing` after the rename). It is a reference implementation and test fake, not a storage choice, and must not appear in a customer dependency list as a peer of `postgres-store`.
...
**Target state.** One package, split entry points: `@agent-chat-platform/chat-ui/shell` (chat + login) and `@agent-chat-platform/chat-ui/admin` (superadmin/governance). `chat-widget` imports only the shell entry. The full control-plane package split stays deferred per the working notes, until governance grows beyond the current panel.
